"""V13 P1 evaluation — 20-frame comparison: V1 vs V12 vs V13.

P1 improvements (over V12):
  P1-2: Multi-dimensional frame features (snr_diff, snr_mag, diff_concentration)
        7-tier (p95 × snr) classification replacing 5-tier p95-only
        SNR-guarded V1 fallback — only when signal truly stands out
  P1-1: Grid-search-optimized parameters
        edge_bonus adapts per tier, refined ratio/expand thresholds
"""
...
EVAL_OUT = PROJECT_ROOT / "output" / "evaluation" / "v13_p1_test"
EVAL_OUT.mkdir(parents=True, exist_ok=True)

# ════════════════════════════════════════════════════════════════════
# 20-frame selection (same as V12 for fair comparison)
# ════════════════════════════════════════════════════════════════════
...
print("=" * 100)
print("  V13 P1 IMPROVEMENTS — 20-FRAME COMPARISON")
print(f"  Methods: V1 (seed_expand) | V12 (P0:CCA+signal+seed-check) | V13 (P1:multi-feature+grid-search)")
...
methods = {
    ...
    "V13": ("seed_expand_v13",  lambda pg, cg: detect_motion_seed_expand_v13(pg, cg)),
}
...
for i, fid in enumerate(ALL_FRAMES):
    ...
    frame_stats[fid] = {"p95": p95, "p50": p50, "snr": snr, "gt_px": gt_px}
    ...
    print(f"  [{i + 1}/{len(ALL_FRAMES)}] {fid}  P95={p95:.1f}  SNR={snr:.2f}  GT={gt_px:,}px")
...
header = f"{'Frame':<8} {'P95':>6} {'SNR':>6} {'GT_px':>7} |"
for m in ["V1", "V12", "V13"]:
    header += f" {'F1_' + m:>8} {'Prec_' + m:>8} {'Rec_' + m:>8} |"
header += f" {'Best':>8} {'ΔV13-V12':>9} {'ΔV13-V1':>9}"
print(header)
print("-" * len(header))

for fid in ALL_FRAMES:
    ps = frame_stats[fid]
    row = f"{fid:<8} {ps['p95']:>6.1f} {ps['snr']:>6.2f} {ps['gt_px']:>7,} |"

    best_f1 = -1
    best_m = ""
    f1s = {}
    for mname in ["V1", "V12", "V13"]:
...
v13_f1s = [all_results["V13"][f]["f1"] for f in ALL_FRAMES]
...
print(f"  V13 mean F1:  {np.mean(v13_f1s):.4f}  (std: {np.std(v13_f1s):.4f})")
print(f"  Δ V13–V12:    {np.mean(v13_f1s) - np.mean(v12_f1s):+.4f}")
print(f"  Δ V13–V1:     {np.mean(v13_f1s) - np.mean(v1_f1s):+.4f}")

# Win counts
v13_wins = sum(1 for i, f in enumerate(ALL_FRAMES)
               if v13_f1s[i] >= max(v1_f1s[i], v12_f1s[i]))
v12_wins = sum(1 for i, f in enumerate(ALL_FRAMES)
               if v12_f1s[i] > max(v1_f1s[i], v13_f1s[i]))
v1_wins = sum(1 for i, f in enumerate(ALL_FRAMES)
              if v1_f1s[i] > max(v12_f1s[i], v13_f1s[i]))

print(f"\n  Frame wins:  V1={v1_wins}  V12={v12_wins}  V13={v13_wins}  (ties to V13)")

# FP/TP analysis
v13_tps = sum(all_results["V13"][f]["tp"] for f in ALL_FRAMES)
v13_fps = sum(all_results["V13"][f]["fp"] for f in ALL_FRAMES)
...
print(f"\n  Total TP:    V1={v1_tps:,}  V12={v12_tps:,}  V13={v13_tps:,}")
print(f"  Total FP:    V1={v1_fps:,}  V12={v12_fps:,}  V13={v13_fps:,}")
print(f"  FP vs V12:   {(v13_fps / v12_fps - 1) * 100:+.1f}%")
print(f"  FP vs V1:    {(v13_fps / v1_fps - 1) * 100:+.1f}%  "
      f"({(1 - v13_fps / v1_fps) * 100:.1f}% reduction)")
print(f"  TP vs V12:   {(v13_tps / v12_tps - 1) * 100:+.1f}%")
print(f"  TP vs V1:    {(v13_tps / v1_tps - 1) * 100:+.1f}%")

# ════════════════════════════════════════════════════════════════════
# Per-frame delta detail (significant changes)
# ════════════════════════════════════════════════════════════════════
print("\n\n" + "=" * 100)
print("  PER-FRAME DELTA: V13 vs V12 (significant changes only)")
print("=" * 100)
print(f"{'Frame':<8} {'P95':>6} {'SNR':>6} {'ΔF1':>9} {'ΔPrec':>9} {'ΔRec':>9}  "
      f"{'ΔTP':>9} {'ΔFP':>9} {'ΔFN':>9}")
print("-" * 85)

for fid in ALL_FRAMES:
    v13 = all_results["V13"][fid]
    v12 = all_results["V12"][fid]
    df1 = v13["f1"] - v12["f1"]
    dp = v13["precision"] - v12["precision"]
    dr = v13["recall"] - v12["recall"]
    dtp = v13["tp"] - v12["tp"]
    dfp = v13["fp"] - v12["fp"]
    dfn = v13["fn"] - v12["fn"]
    if abs(df1) > 0.0005:
        ps = frame_stats[fid]
        print(f"{fid:<8} {ps['p95']:>6.1f} {ps['snr']:>6.2f} {df1:>+9.4f} {dp:>+9.4f} {dr:>+9.4f}  "
              f"{dtp:>+9,} {dfp:>+9,} {dfn:>+9,}")
...
# ════════════════════════════════════════════════════════════════════
print("\n\n" + "=" * 100)
print("  MOTION STRENGTH GROUP ANALYSIS (by P95)")
print("=" * 100)
...
for gname, gframes in groups.items():
    ...
    for mname in ["V1", "V12", "V13"]:
        ...
        if f1s_g:
            best_marker = " <<<" if mname == "V13" else ""
            print(f"  {mname:<8} {np.mean(f1s_g):>10.4f} {np.mean(precs_g):>10.4f} "
                  f"{np.mean(recs_g):>10.4f} {fps_g:>10,}{best_marker}")

# ════════════════════════════════════════════════════════════════════
# SNR-based Group Analysis (key P1 insight)
# ════════════════════════════════════════════════════════════════════
print("\n\n" + "=" * 100)
print("  SNR GROUP ANALYSIS (P1-2 key metric)")
print("=" * 100)

snr_groups = {
    "High SNR (>2.5)":       [f for f in ALL_FRAMES if frame_stats[f]["snr"] > 2.5],
    "Med-High SNR (2.0-2.5)": [f for f in ALL_FRAMES if 2.0 < frame_stats[f]["snr"] <= 2.5],
    "Medium SNR (1.6-2.0)":  [f for f in ALL_FRAMES if 1.6 < frame_stats[f]["snr"] <= 2.0],
    "Low SNR (<=1.6)":       [f for f in ALL_FRAMES if frame_stats[f]["snr"] <= 1.6],
}

for gname, gframes in snr_groups.items():
    if not gframes:
        continue
...
for i, fid in enumerate(ALL_FRAMES):
    ...
    m_v13 = detect_motion_seed_expand_v13(pg, cg)

    # Metrics
    s_v1 = all_results["V1"][fid]
    s_v12 = all_results["V12"][fid]
    s_v13 = all_results["V13"][fid]

    # Build visualization
    ROW_H = 220

    # Row 0: Original image + Ground Truth + GT Overlay
    ...
    put_label(err_v13_rs, "V13 (P1)")
    put_label(err_v13_rs, f"F1={s_v13['f1']:.3f} P={s_v13['precision']:.3f} R={s_v13['recall']:.3f}", 44)
    put_label(err_v13_rs, f"TP={s_v13['tp']:,} FP={s_v13['fp']:,} FN={s_v13['fn']:,}", 66)

    row1 = np.hstack([err_v1_rs, err_v12_rs, err_v13_rs])
    ...
    # Delta info bar
    d_f1_v12 = s_v13["f1"] - s_v12["f1"]
    d_f1_v1 = s_v13["f1"] - s_v1["f1"]
    bar_text = f"V13 vs V12: dF1={d_f1_v12:+.4f}  dFP={s_v13['fp']-s_v12['fp']:+,}  |  V13 vs V1: dF1={d_f1_v1:+.4f}  dFP={s_v13['fp']-s_v1['fp']:+,}"
    ...
    out_path = PER_FRAME_DIR / f"{fid}_v13_comparison.png"
