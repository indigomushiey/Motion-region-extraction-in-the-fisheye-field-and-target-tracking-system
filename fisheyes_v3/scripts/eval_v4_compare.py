"""50-frame V4 comparison: V3 vs V4, with visualization."""
import cv2, numpy as np, sys, importlib.util, time, gc
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
V2_ROOT = Path(__file__).resolve().parents[2] / "fisheyes_v2" / "fisheye_motion_tracking"
sys.path.insert(0, str(PROJECT_ROOT))
from core.motion_hybrid import (detect_motion_hybrid_v3, detect_motion_hybrid_v4)

def _i(rp, nm):
    fp = V2_ROOT / rp
    s = importlib.util.spec_from_file_location(nm, fp)
    m = importlib.util.module_from_spec(s); s.loader.exec_module(m); return m

cm = _i("core/evaluation/metrics.py", "mt").compute_metrics

DATA = PROJECT_ROOT / "data" / "homework2"
GTD = DATA / "motion_annotation" / "GroudTruth"
CUR = DATA / "rgb_images"; PRV = DATA / "previous_images"
OUT = PROJECT_ROOT / "output" / "evaluation" / "v4_compare"
OUT.mkdir(parents=True, exist_ok=True)
PF = OUT / "per_frame"; PF.mkdir(parents=True, exist_ok=True)

# ── Sample 50 frames ──
gt_files = sorted(GTD.glob("*_FV.png"))
all_frames = [f.stem.replace("_FV", "") for f in gt_files
              if (CUR / f"{f.stem.replace('_FV', '')}_FV.png").exists()
              and (PRV / f"{f.stem.replace('_FV', '')}_FV_prev.png").exists()]
N = len(all_frames); SAMPLE = 50
step = N / SAMPLE
sampled = [all_frames[int(i * step)] for i in range(SAMPLE)]
if all_frames[-1] not in sampled:
    sampled[-1] = all_frames[-1]

print(f"{'='*80}")
print(f"  V4 COMPARISON: V3 vs V4  ({SAMPLE} frames)")
print(f"  V4 adds: per-frame adaptive threshold (robust z-score)")
print(f"{'='*80}")

methods = {
    "V3": lambda pg, cg: detect_motion_hybrid_v3(pg, cg),
    "V4": lambda pg, cg: detect_motion_hybrid_v4(pg, cg),
}

all_results = {n: {} for n in methods}
frame_stats = {}
times = {n: [] for n in methods}

print(f"\n{'Frame':<8} {'P95':>6} {'GT_px':>8}  {'V3 F1':>8} {'V4 F1':>8} {'Δ':>9} {'Best':>6}")
print("-" * 60)

for idx, fid in enumerate(sampled):
    c = cv2.imread(str(CUR / f"{fid}_FV.png"))
    p = cv2.imread(str(PRV / f"{fid}_FV_prev.png"))
    g = cv2.imread(str(GTD / f"{fid}_FV.png"), cv2.IMREAD_GRAYSCALE)
    pg = cv2.cvtColor(p, cv2.COLOR_BGR2GRAY)
    cg = cv2.cvtColor(c, cv2.COLOR_BGR2GRAY)
    gt = (g > 0).astype(np.uint8) * 255

    p95 = np.percentile(cv2.absdiff(cg, pg), 95)
    gt_px = int(gt.sum() / 255)
    frame_stats[fid] = {"p95": p95, "gt_px": gt_px}

    for mn, fn in methods.items():
        t0 = time.perf_counter()
        mask = fn(pg, cg)
        times[mn].append(time.perf_counter() - t0)
        all_results[mn][fid] = cm(mask, gt)

    v3f = all_results["V3"][fid]["f1"]
    v4f = all_results["V4"][fid]["f1"]
    d = v4f - v3f
    best = "V4" if d >= 0 else "V3"
    print(f"{fid:<8} {p95:>6.0f} {gt_px:>8,}  {v3f:>8.4f} {v4f:>8.4f} {d:>+9.4f} {best:>6}")

    if (idx + 1) % 10 == 0:
        v3_m = np.mean([all_results["V3"][f]["f1"] for f in sampled[:idx+1]])
        v4_m = np.mean([all_results["V4"][f]["f1"] for f in sampled[:idx+1]])
        print(f"  --- [{idx+1}/{SAMPLE}] running avg: V3={v3_m:.4f}  V4={v4_m:.4f}  Δ={v4_m-v3_m:+.4f} ---")
    gc.collect()

# ── Summary ──
v3_f1s = [all_results["V3"][f]["f1"] for f in sampled]
v4_f1s = [all_results["V4"][f]["f1"] for f in sampled]

print(f"\n{'='*80}")
print("  SUMMARY")
print(f"{'='*80}")
print(f"{'Method':<8} {'F1':>10} {'Precision':>10} {'Recall':>10} {'TP':>12} {'FP':>12} {'Time/f':>8}")
print("-" * 80)

for mn in ["V3", "V4"]:
    ml = [all_results[mn][f] for f in sampled]
    tp_s = sum(r["tp"] for r in ml)
    fp_s = sum(r["fp"] for r in ml)
    avg_f1 = np.mean([r["f1"] for r in ml])
    avg_p  = tp_s / (tp_s + fp_s + 1)
    avg_r  = np.mean([r["recall"] for r in ml])
    avg_t  = np.mean(times[mn])
    mk = "  <<<" if mn == "V4" else ""
    print(f"{mn:<8} {avg_f1:>10.4f} {avg_p:>10.4f} {avg_r:>10.4f} {tp_s:>12,} {fp_s:>12,} {avg_t:>7.2f}s{mk}")

print(f"\n  Δ V4-V3: F1={np.mean(v4_f1s)-np.mean(v3_f1s):+.4f}")
v3_win = sum(1 for i in range(SAMPLE) if v3_f1s[i] > v4_f1s[i])
v4_win = sum(1 for i in range(SAMPLE) if v4_f1s[i] >= v3_f1s[i])
print(f"  Frame wins: V3={v3_win}  V4={v4_win}")

# Top deltas
deltas = [(fid, v4_f1s[i] - v3_f1s[i]) for i, fid in enumerate(sampled)]
deltas.sort(key=lambda x: x[1])
print(f"\n  Top-5 V4 wins:  {', '.join(f'{f}({d:+.4f})' for f,d in deltas[-5:][::-1])}")
print(f"  Top-5 V3 wins:  {', '.join(f'{f}({d:+.4f})' for f,d in deltas[:5])}")

# ── Save text ──
with open(OUT / "v4_compare_results.txt", "w", encoding="utf-8") as f:
    f.write(f"{'Frame':<8} {'P95':>6} {'GT_px':>8}  {'V3_F1':>8} {'V4_F1':>8} {'Δ':>9} {'Best':>6}\n")
    f.write("-" * 60 + "\n")
    for fid in sampled:
        s = frame_stats[fid]
        v3f = all_results["V3"][fid]["f1"]
        v4f = all_results["V4"][fid]["f1"]
        f.write(f"{fid:<8} {s['p95']:>6.0f} {s['gt_px']:>8,}  {v3f:>8.4f} {v4f:>8.4f} {v4f-v3f:>+9.4f} {'V4' if v4f>=v3f else 'V3':>6}\n")
print(f"\nResults: {OUT / 'v4_compare_results.txt'}")

# ── Visualization ──
def color_error(pred, gt):
    pb, gb = (pred > 0), (gt > 0)
    out = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
    out[pb & gb] = [0, 255, 0]; out[pb & ~gb] = [0, 0, 255]; out[~pb & gb] = [255, 0, 0]
    return out

def rh(img, th):
    r = th / img.shape[0]
    return cv2.resize(img, (int(img.shape[1] * r), th))

def pl(img, txt, y=22, c=(255, 255, 255)):
    cv2.putText(img, txt, (6, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 2)
    cv2.putText(img, txt, (5, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, c, 1)

print(f"\n{'='*80}")
print("  GENERATING VISUALIZATIONS...")
print(f"{'='*80}")

RH_ = 200
for idx, fid in enumerate(sampled):
    c = cv2.imread(str(CUR / f"{fid}_FV.png"))
    p = cv2.imread(str(PRV / f"{fid}_FV_prev.png"))
    g = cv2.imread(str(GTD / f"{fid}_FV.png"), cv2.IMREAD_GRAYSCALE)
    pg = cv2.cvtColor(p, cv2.COLOR_BGR2GRAY); cg = cv2.cvtColor(c, cv2.COLOR_BGR2GRAY)
    gb = (g > 0)

    mv3 = detect_motion_hybrid_v3(pg, cg)
    mv4 = detect_motion_hybrid_v4(pg, cg)

    s = frame_stats[fid]
    r3 = all_results["V3"][fid]; r4 = all_results["V4"][fid]

    # Row 0: Input
    curr_s = rh(c, RH_)
    gt_ov = curr_s.copy(); gm = rh((gb.astype(np.uint8) * 255), RH_)
    gt_ov[gm > 0] = (gt_ov[gm > 0] * 0.4 + np.array([0, 255, 0]) * 0.6).astype(np.uint8)
    r0 = np.hstack([curr_s, gt_ov, rh(cv2.applyColorMap(cv2.absdiff(cg, pg), cv2.COLORMAP_HOT), RH_)])
    pl(r0, f"{fid}  P95={s['p95']:.0f}  GT={s['gt_px']:,}px")

    # Row 1: Error maps
    e3 = rh(color_error(mv3, gb), RH_); e4 = rh(color_error(mv4, gb), RH_)
    r1 = np.hstack([e3, e4])
    pl(r1, f"V3  F1={r3['f1']:.4f}  P={r3['precision']:.4f}  R={r3['recall']:.4f}")
    pl(r1[:, e3.shape[1]:], f"V4  F1={r4['f1']:.4f}  P={r4['precision']:.4f}  R={r4['recall']:.4f}")
    pl(e3, f"TP={r3['tp']:,} FP={r3['fp']:,}", 42)
    pl(e4, f"TP={r4['tp']:,} FP={r4['fp']:,}", 42)

    # Row 2: Pred masks
    p3 = rh(cv2.cvtColor(mv3, cv2.COLOR_GRAY2BGR), RH_)
    p4 = rh(cv2.cvtColor(mv4, cv2.COLOR_GRAY2BGR), RH_)
    r2 = np.hstack([p3, p4])

    rows = [r0, r1, r2]; mw = max(r.shape[1] for r in rows)
    padded = []
    for r in rows:
        pad = np.zeros((r.shape[0], mw - r.shape[1], 3), dtype=np.uint8) if r.shape[1] < mw else np.zeros((r.shape[0], 0, 3), dtype=np.uint8)
        padded.append(np.hstack([r, pad]) if pad.shape[1] > 0 else r)

    lg = np.zeros((26, mw, 3), dtype=np.uint8); lg[:] = [30, 30, 30]
    for j, (txt, col) in enumerate([("Green=TP", (0, 255, 0)), ("Red=FP", (0, 0, 255)), ("Blue=FN", (255, 0, 0))]):
        cv2.putText(lg, txt, (10 + j * mw // 3, 19), cv2.FONT_HERSHEY_SIMPLEX, 0.38, col, 2)

    db = np.zeros((22, mw, 3), dtype=np.uint8); db[:] = [45, 45, 45]
    dt = f"Δ V4-V3: F1={r4['f1']-r3['f1']:+.4f}  TP={r4['tp']-r3['tp']:+,}  FP={r4['fp']-r3['fp']:+,}"

    cv2.putText(db, dt, (10, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (200, 200, 200), 1)
    viz = np.vstack([padded[0], padded[1], padded[2], lg, db])
    cv2.imwrite(str(PF / f"{fid}_v4compare.png"), viz)
    print(f"  [{idx+1:>2}/{SAMPLE}] {fid}")

print(f"\n  Images: {PF}/")

# ── Chart ──
import matplotlib; matplotlib.use('Agg'); import matplotlib.pyplot as plt

fig, axes = plt.subplots(2, 2, figsize=(18, 10))
fig.suptitle(f'V4 vs V3: Adaptive Z-Score Threshold  |  '
             f'V3 F1={np.mean(v3_f1s):.4f}  V4 F1={np.mean(v4_f1s):.4f}  Δ={np.mean(v4_f1s)-np.mean(v3_f1s):+.4f}',
             fontsize=13, fontweight='bold')

x = np.arange(SAMPLE); w = 0.35

ax = axes[0, 0]
ax.bar(x - w/2, v3_f1s, w, label='V3', color='#2ecc71', alpha=0.85)
ax.bar(x + w/2, v4_f1s, w, label='V4', color='#9b59b6', alpha=0.85)
ax.set_title('F1 per Frame'); ax.legend(fontsize=8)
ax.set_xticks(x[::5]); ax.set_xticklabels([sampled[i] for i in range(0, SAMPLE, 5)], rotation=45, ha='right', fontsize=6)
ax.grid(axis='y', alpha=0.3)

ax = axes[0, 1]
d_v4v3 = [v4_f1s[i] - v3_f1s[i] for i in range(SAMPLE)]
colors = ['#27ae60' if d >= 0 else '#e74c3c' for d in d_v4v3]
ax.bar(x, d_v4v3, 0.6, color=colors, alpha=0.85)
ax.axhline(y=0, color='black', linewidth=0.8)
ax.set_title('Δ F1 (V4 - V3)'); ax.set_xticks(x[::5])
ax.set_xticklabels([sampled[i] for i in range(0, SAMPLE, 5)], rotation=45, ha='right', fontsize=6)
ax.grid(axis='y', alpha=0.3)

ax = axes[1, 0]
for mn, c, m in [("V3", '#2ecc71', 'o'), ("V4", '#9b59b6', 's')]:
    pp = [all_results[mn][f]["precision"] for f in sampled]
    rr = [all_results[mn][f]["recall"] for f in sampled]
    ax.scatter(pp, rr, c=c, label=mn, alpha=0.6, s=25, marker=m)
ax.set_xlabel('Precision'); ax.set_ylabel('Recall'); ax.set_title('Precision-Recall')
ax.legend(fontsize=8); ax.grid(alpha=0.3)

ax = axes[1, 1]
p95s = [frame_stats[f]["p95"] for f in sampled]
ax.scatter(p95s, v3_f1s, c='#2ecc71', label='V3', alpha=0.4, s=20)
ax.scatter(p95s, v4_f1s, c='#9b59b6', label='V4', alpha=0.5, s=20)
ax.set_xlabel('P95 Diff'); ax.set_ylabel('F1'); ax.set_title('F1 vs Difficulty')
ax.legend(fontsize=8); ax.grid(alpha=0.3)

plt.tight_layout()
chart_path = OUT / "v4_compare_chart.png"
fig.savefig(str(chart_path), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"  Chart: {chart_path}")
print(f"\n{'='*80}")
print("  DONE!")
print(f"{'='*80}")
