"""Magnitude Blending evaluation: V1 vs V12 vs Blend on 20 frames."""
...
from core.motion_blend import detect_motion_blend
...
OUT_DIR = PROJECT_ROOT / "output" / "evaluation" / "blend"
...
print("=" * 100)
print("  MAGNITUDE BLENDING: Center=persp mag, Edge=fish mag")
print("  Methods: V1 | V12 | BL (blended)")
...
methods = {
    ...
    "BL":  lambda pg, cg: detect_motion_blend(pg, cg),
}
...
for idx, fid in enumerate(ALL_FRAMES):
    ...
    v1f, v12f, blf = (all_results[m][fid]['f1'] for m in ["V1", "V12", "BL"])
    print(f"  [{idx+1}/{len(ALL_FRAMES)}] {fid}  P95={frame_stats[fid]['p95']:.0f}"
          f"  V1={v1f:.3f}  V12={v12f:.3f}  BL={blf:.3f}")
print("Done.\n")
...
for m in ["V1", "V12", "BL"]: hdr += f" {'F1_'+m:>8} {'P_'+m:>8} {'R_'+m:>8} |"
hdr += f" {'Best':>8} {'ΔBL-12':>9} {'ΔBL-V1':>9}"
...
for fid in ALL_FRAMES:
    ...
    row += f" {best_m:>8} {f1s['BL']-f1s['V12']:>+9.4f} {f1s['BL']-f1s['V1']:>+9.4f}"
    print(row)
...
print("=" * 100)
bl_f = [all_results["BL"][f]["f1"] for f in ALL_FRAMES]
...
print(f"  BL  mean F1: {np.mean(bl_f):.4f}  (std: {np.std(bl_f):.4f})")
print(f"  Δ BL–V12:    {np.mean(bl_f)-np.mean(v12_f):+.4f}")
print(f"  Δ BL–V1:     {np.mean(bl_f)-np.mean(v1_f):+.4f}")
...
print(f"\n  Frame wins: V1={v1_w}  V12={v12_w}  BL={bl_w}")

bl_tp = sum(all_results["BL"][f]["tp"] for f in ALL_FRAMES)
bl_fp = sum(all_results["BL"][f]["fp"] for f in ALL_FRAMES)
v12_tp = sum(all_results["V12"][f]["tp"] for f in ALL_FRAMES)
v12_fp = sum(all_results["V12"][f]["fp"] for f in ALL_FRAMES)
print(f"\n  TP: V12={v12_tp:,}  BL={bl_tp:,}  ({(bl_tp/v12_tp-1)*100:+.1f}%)")
print(f"  FP: V12={v12_fp:,}  BL={bl_fp:,}  ({(bl_fp/v12_fp-1)*100:+.1f}%)")

# ═══════════════════════════════════════════════════════════════════
# Per-frame significant deltas
# ═══════════════════════════════════════════════════════════════════
print("\n\n" + "=" * 100)
print("  BL vs V12 — Significant Changes")
print("=" * 100)
print(f"{'Frame':<8} {'P95':>6} {'ΔF1':>9} {'ΔPrec':>9} {'ΔRec':>9}  "
      f"{'ΔTP':>9} {'ΔFP':>9} {'ΔFN':>9}")
print("-" * 80)
for fid in ALL_FRAMES:
    bl, v12 = all_results["BL"][fid], all_results["V12"][fid]
    df1 = bl["f1"] - v12["f1"]
    if abs(df1) > 0.0005:
...
for idx, fid in enumerate(ALL_FRAMES):
    ...
    mbl = detect_motion_blend(pg, cg)

    s = {m: all_results[m][fid] for m in ["V1","V12","BL"]}
    ...
    for e, mn in [(ev1,"V1"),(ev12,"V12"),(ebl,"BL")]:
    ...
    dt = f"BL vs V12: dF1={s['BL']['f1']-s['V12']['f1']:+.4f}  dFP={s['BL']['fp']-s['V12']['fp']:+,}"
    cv2.putText(db, dt, (10,18), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,200), 1)

    viz = np.vstack([r0, r1, lg, db])
    op = PF / f"{fid}_blend.png"; cv2.imwrite(str(op), viz)
