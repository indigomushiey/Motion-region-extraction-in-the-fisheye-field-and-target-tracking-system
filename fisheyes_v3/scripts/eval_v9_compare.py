"""50-frame V9: V7 vs V9 (local contrast norm + seed direction filter)."""
...
OUT = PROJECT_ROOT / "output" / "evaluation" / "v9_compare"
...
print(f"  V9: Local Contrast Norm + Seed Filter  ({SAMPLE} frames)")

methods = {"V7": lambda pg,cg: detect_motion_hybrid_v7(pg,cg),
           "V9": lambda pg,cg: detect_motion_hybrid_v9(pg,cg)}
results = {n:{} for n in methods}; stats = {}; times = {n:[] for n in methods}

print(f"\n{'Frame':<8} {'P95':>6} {'GT_px':>8}  {'V7 F1':>8} {'V9 F1':>8} {'Δ':>9} {'Best':>6}  {'V7 FP':>10} {'V9 FP':>10}")
print("-"*95)
...
for idx, fid in enumerate(sampled):
    ...
    r7=results["V7"][fid]; r9=results["V9"][fid]; d=r9["f1"]-r7["f1"]
    print(f"{fid:<8} {p95:>6.0f} {stats[fid]['gt_px']:>8,}  {r7['f1']:>8.4f} {r9['f1']:>8.4f} {d:>+9.4f} {'V9' if d>=0 else 'V7':>6}  {r7['fp']:>10,} {r9['fp']:>10,}")
    if (idx+1)%10==0:
        v7m=np.mean([results["V7"][f]["f1"] for f in sampled[:idx+1]])
        v9m=np.mean([results["V9"][f]["f1"] for f in sampled[:idx+1]])
        print(f"  --- [{idx+1}/{SAMPLE}] V7={v7m:.4f} V9={v9m:.4f} Δ={v9m-v7m:+.4f} ---")
    gc.collect()

f7=[results["V7"][f]["f1"] for f in sampled]; f9=[results["V9"][f]["f1"] for f in sampled]
print(f"\n{'='*80}  SUMMARY")
for mn in ["V7","V9"]:
...
print(f"  Δ V9-V7: {np.mean(f9)-np.mean(f7):+.4f}  |  wins: V7={sum(1 for i in range(SAMPLE) if f7[i]>f9[i])}  V9={sum(1 for i in range(SAMPLE) if f9[i]>=f7[i])}")
ds=[(fid,f9[i]-f7[i]) for i,fid in enumerate(sampled)]; ds.sort(key=lambda x:x[1])
print(f"  Top-5 V9: {', '.join(f'{f}({d:+.4f})' for f,d in ds[-5:][::-1])}")
print(f"  Top-5 V7: {', '.join(f'{f}({d:+.4f})' for f,d in ds[:5])}")

# Save text
with open(OUT/"v9_compare_results.txt","w",encoding="utf-8") as f:
    for fid in sampled:
        s=stats[fid]; r7=results["V7"][fid]; r9=results["V9"][fid]
        f.write(f"{fid} {s['p95']:.0f} {s['gt_px']} {r7['f1']:.4f} {r9['f1']:.4f} {r9['f1']-r7['f1']:+.4f} {r7['fp']} {r9['fp']}\n")
print(f"  DONE!")
